          ...
          <section id="workflow-ablauf" aria-labelledby="workflow-heading" className="scroll-mt-32">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-8">
              {workflowSteps.map((step) => (
                <article key={step.title} className="p-8 rounded-3xl bg-gray-50 border border-gray-100">
                  <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">{step.title}</p>
            ...
            <div className="flex flex-wrap gap-3 mb-8">
              {integrationChips.map((chip) => (
                <span
                  key={chip}
                  className="inline-flex items-center rounded-full border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-700"
                >
                  {chip}
                </span>
              ))}
