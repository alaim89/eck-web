...
export default function SecurityComplianceCaseStudy() {
  const { t } = useLanguage();
  const caseStudy = t.caseStudies.items[3];
        ...
        <div className="max-w-[800px] mx-auto">
          ...
          <div className="mb-16">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gray-50 text-gray-700 text-xs font-semibold uppercase tracking-wider mb-6">
              {t.caseStudies.badge}: {caseStudy.company}
            </div>
              ...
              <div className="grid sm:grid-cols-3 gap-6 mb-8">
                {details.result.metrics.map((metric: any, i: number) => (
                  <div key={i} className="p-6 bg-gray-50 rounded-2xl border border-gray-100 text-center">
                    {i === 0 && <TrendingUp className="w-8 h-8 text-primary mx-auto mb-3" />}
                    {i === 1 && <ShieldCheck className="w-8 h-8 text-primary mx-auto mb-3" />}
                    {i === 2 && <Clock className="w-8 h-8 text-primary mx-auto mb-3" />}
